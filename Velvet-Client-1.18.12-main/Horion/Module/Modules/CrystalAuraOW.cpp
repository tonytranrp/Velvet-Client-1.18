...
void findEntity16(C_Entity* currentEntity16, bool isRegularEntity) {
	...
	if (currentEntity16->getNameTag()->getTextLength() <= 1 && currentEntity16->getEntityTypeId() == 319)
		return;
...
bool CrystalAuraOW::tryCrystalAuraOW(vec3_t CrystalAuraOW) {
	...
	if (blockLegacy->material->isReplaceable) {
		...
		if (foundCandidate) {
			g_Data.getCGameMode()->buildBlock(&blok, 1);
			g_Data.getCGameMode()->buildBlock(&blok, 2);
...
void CrystalAuraOW::onTick(C_GameMode* gm) {
	...
	int crystal = 0;
	int nukkitcrsytal = 637;

	crystal = 629;

	if (!targetList16.empty()) {
		if (g_Data.getLocalPlayer()->getSelectedItemId() == crystal, nukkitcrsytal) {
			// ground level
