...
pvpResources::pvpResources() : IModule(0x0, Category::PLAYER, "Displays items on screen that are used for pvp") {
...
void pvpResources::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	if (!text) {
		...
		if (isThereShield()) {
			...
			std::string yikes = std::to_string(countItem(355));
		...
		if (Totem) {
			...
			DrawUtils::drawText(vec2_t(pvpX, pvpY), &std::string("Totems: " + totems), MC_Color(255, 255, 130), scale);
		}
		// xp bottles
		std::string xp = std::to_string(countItem(355));
		DrawUtils::drawText(vec2_t(pvpX, pvpY + 10.f * scale), &std::string("Shield: " + xp), MC_Color(160, 255, 0), scale);
		// Gapples
		...
		DrawUtils::drawText(vec2_t(pvpX, pvpY + 40.f * scale), &std::string("Obsidian: " + obsidians), MC_Color(28, 95, 246), scale);
	}
}
...
bool pvpResources::isThereShield() {
	int mafs = 0;
	for (int i = 0; i < 36; i++) {
		int itemIDC = 355;
		C_ItemStack* itemToCount = g_Data.getLocalPlayer()->getSupplies()->inventory->getItemStack(i);
