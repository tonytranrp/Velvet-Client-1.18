...
CrystalAuraWTA::CrystalAuraWTA() : IModule(0x0, Category::COMBAT, "CrystalAura by XxTrongDZxX") {
...
std::vector<vec3_t*> getGucciPlacement2(C_Entity* ent) {
	...
	for (auto blocks : blockChecks) {
		...
		if (!checkTargCollision(&blocks->toVector3(), ent)) {  // very efficient coding here
			/*
...
void findCr() {
	...
	// return (supplies->selectedHotbarSlot = slotab);
}
			...
			if ((crystalDelay >= this->delay) && !(targetList7.empty())) {
				...
				if (!checkSurrounded2(targetList7[0])) {
					...
					if (!gucciPositions.empty()) {
						...
						if (silent) supplies->selectedHotbarSlot = slotab;
					}
