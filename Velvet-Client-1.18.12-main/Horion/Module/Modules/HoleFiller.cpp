...
HoleFiller::HoleFiller() : IModule(0x0, Category::COMBAT, "Locate a good place to holecamp ;)") {
	
	mode = SettingEnum(this).addEntry(EnumEntry("Obsidian", 0)).addEntry(EnumEntry("Coweb", 1));
...
bool HoleFiller::canpla(vec3_ti blockPos) {
...
void findEntity19(C_Entity* currentEntity19, bool isRegularEntity) {
	...
	if (!Target::isValidTarget(currentEntity19))
		return;
...
bool HoleFiller::tryHoleFiller(vec3_ti blkPlacement) {
