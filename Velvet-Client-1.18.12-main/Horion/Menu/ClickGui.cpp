...
static constexpr float textSize = 0.783f;
...
static const MC_Color enabledModuleColor = MC_Color(150, 194, 205);
static const MC_Color whiteColor = MC_Color(245, 240, 217);
static const MC_Color selectedModuleColor = MC_Color(0, 0, 0);
...
static const MC_Color SettingColor1 = MC_Color(145, 173, 180);  // not enabled enums
static const MC_Color SettingColor2 = MC_Color(145, 173, 180);  // enabled enums and enums when hovering over with mouse
...
void ClickGui::renderCategory(Category category) {
	...
	if (ourWindow->isExtended || ourWindow->isInAnimation) {
		...
		const float cutoffHeight = roundf(g_Data.getGuiData()->heightGame * 0.90f) + 0.5f /*fix flickering related to rounding errors*/;
		int moduleIndex = 0;
		for (auto& mod : moduleList) {
			...
			if (allowRender) {
				if (!ourWindow->isInAnimation && !isDragging && rectPos.contains(&mousePos)) {
					...
					DrawUtils::fillRectangle(rectPos, mod->isEnabled() ? enabledModuleColor : moduleColor, 0.2f);  // background
				}
			}
			...
			// Settings
			{
				...
				if (settings->size() > 2 && allowRender) {
					...
					// MC_Color(0, 216, 0), crossWidth, crossSize, !clickMod->isExtended);
						...
						for (auto setting : *settings) {
							...
							case ValueType::ENUM_T: {
								...
								// Text and background
								{
									...
									// currentXOffset + windowSize->x + paddingRight - (crossSize / 2) - 1.f,
									// currentYOffset + textPadding + (textHeight / 2)),
									// MC_Color(245, 239, 245), crossWidth, crossSize, !setting->minValue->_bool);
									if (rectPos.contains(&mousePos) && shouldToggleRightClick && !ourWindow->isInAnimation) {
										shouldToggleRightClick = false;
										setting->minValue->_bool = !setting->minValue->_bool;
									}
							...
							case ValueType::FLOAT_T: {
								...
								// Slider
								{
									...
									// RENDERs & Logic
									{
										...
										DrawUtils::drawRectangle(rect, whiteColor, 1.f, 0.2f);  // Slider outline
										...
										// Draw Progress
										{
											...
											rect.z = rect.x + value;
											DrawUtils::fillRectangle(rect, MC_Color(85, 85, 85), (areWeFocused || setting->isDragging) ? 1.f : 0.8f);  // progress bar 2.0
										}
							...
							case ValueType::INT_T: {
								...
								// Slider
								{
									...
									// RENDERs & Logic
									{
										...
										DrawUtils::drawRectangle(rect, MC_Color(245, 160, 5), textSize);  // Slider outline
										...
										// Draw Progress
										{
											...
											rect.z = rect.x + value;
											DrawUtils::fillRectangle(rect, MC_Color(85, 85, 85), (areWeFocused || setting->isDragging) ? 1.f : 0.8f);  // bar drag thinggy progress
										}
							...
							default: {
								...
								// DrawUtils::drawText(textPos, &elTexto, MC_Color(1.0f, 1.0f, 1.0f), textSize); //not needed in NoHaveHands case
								currentYOffset += textHeight + (textPadding * 2);
							} break;
							}
						}
	...
	// Draw Category Header
	{
		...
		// Draw category component
		{
			...
			DrawUtils::fillRectangle(rectPos, moduleColor, 1.f);                                                                                 // changes color text eg: COMBAT CHAT RENDER
			DrawUtils::fillRectangle(vec4_t(rectPos.x, rectPos.w - 1, rectPos.z, rectPos.w), brightModuleBlendColor, 1 - ourWindow->animation);  // ofc if theres text theres background dumbo
			DrawUtils::drawText(textPos, &textStr, MC_Color(whiteColor), textSize);
			// Draw Dash

			GuiUtils::drawCrossLine(vec2_t(
										currentXOffset + windowSize->x + paddingRight - (crossSize / 2) - 1.f,
										categoryHeaderYOffset + textPadding + (textHeight / 2)),
									MC_Color(255, 216, 0), crossWidth, crossSize, !ourWindow->isExtended);
		}
	}
