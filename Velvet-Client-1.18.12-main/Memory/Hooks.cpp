...
#include "../Horion/Module/Modules/Theme.h"
					...
					if (moduleMgr->isInitialized() && shouldRenderArrayList) {
						...
						for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
							...
							Utils::ColorConvertHSVtoRGB(currColor[0], currColor[1], currColor[2], currColor[0], currColor[1], currColor[2]);
								...
								DrawUtils::fillRectangle(rectPos, MC_Color(12, 12, 12), arrayMod->opacity);
...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, C_Packet* packet) {
	...
	if (autoSneakMod->isEnabled() && g_Data.getLocalPlayer() != nullptr && autoSneakMod->silentlol && packet->isInstanceOf<PlayerActionPacket>()) {
...
void Hooks::Actor__setRot(C_Entity* _this, vec2_t& angle) {
	...
	if (hfMod->isEnabled() && _this == g_Data.getLocalPlayer()) {
		for (int i = 0; i > -90; i--) {
			func(_this, angle = vec2_t(i, 0));
		}
	}
